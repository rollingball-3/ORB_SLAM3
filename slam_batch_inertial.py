...
def run_slam(h5_file, dataset_name, script_dir, use_mask=True):
    ...
    yaml_path = os.path.join(script_dir, "Examples", "RGB-D-Inertial", "orbbec.yaml")
    ...
    dataset_name_inertial = f"{dataset_name}_inertial"
    dataset_dir = os.path.join(log_dir, dataset_name_inertial)
    os.makedirs(dataset_dir, exist_ok=True)
    
    # Generate log filename
    log_file = os.path.join(dataset_dir, f"{dataset_name}_inertial.log")
    
    # Build command
    slam_executable = os.path.join(script_dir, "Examples", "RGB-D-Inertial", "orbbec_bgrd_inertial")
    ...
    print(f"Running Inertial SLAM on dataset: {dataset_name}")
